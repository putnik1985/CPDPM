/****************************************************************************
** Meta object code from reading C++ file 'Axes_editor.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.7)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "Axes_editor.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'Axes_editor.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.7. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Axes_editor[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      19,   13,   12,   12, 0x05,

 // slots: signature, parameters, type, tag, flags
      45,   12,   41,   12, 0x08,
      53,   12,   41,   12, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_Axes_editor[] = {
    "Axes_editor\0\0label\0title_changed(string)\0"
    "int\0label()\0close_dialog()\0"
};

void Axes_editor::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        Axes_editor *_t = static_cast<Axes_editor *>(_o);
        switch (_id) {
        case 0: _t->title_changed((*reinterpret_cast< const string(*)>(_a[1]))); break;
        case 1: { int _r = _t->label();
            if (_a[0]) *reinterpret_cast< int*>(_a[0]) = _r; }  break;
        case 2: { int _r = _t->close_dialog();
            if (_a[0]) *reinterpret_cast< int*>(_a[0]) = _r; }  break;
        default: ;
        }
    }
}

const QMetaObjectExtraData Axes_editor::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject Axes_editor::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_Axes_editor,
      qt_meta_data_Axes_editor, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &Axes_editor::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *Axes_editor::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *Axes_editor::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Axes_editor))
        return static_cast<void*>(const_cast< Axes_editor*>(this));
    return QDialog::qt_metacast(_clname);
}

int Axes_editor::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void Axes_editor::title_changed(const string & _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
